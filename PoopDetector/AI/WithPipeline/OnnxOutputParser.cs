    ...
    (IOnnxModel onnxModel)
{
    ...
    private readonly (float x, float y)[] boxAnchors = onnxModel.Anchors;
    ...
    {
        float cellWidth = InputWidth / columnCount;
        float cellHeight = InputHeight / rowCount;
    ...
    public float[] ExtractClassProbabilities(float[] modelOutput, int row, int column, int channel, float confidence)
    {
        ...
        float[] classProbabilities = new float[ColormapList.Count];
        for (int classProbability = 0; classProbability < ColormapList.Count; classProbability++)
    ...
    public List<BoundingBox> ParseOutputs(float[] modelOutput, float probabilityThreshold = .6f)
    {
        ...
        for (int row = 0; row < rowCount; row++)
        {
            for (int column = 0; column < columnCount; column++)
            {
                ...
                {
                    var channel = box * (ColormapList.Count + featuresPerBox);
                    ...
                    boxes.Add(new BoundingBox
                    {
                        ...
                        Label = ColormapList[topIndex].Item1,
                        BoxColor = ColormapList[topIndex].Item2
                    });
                }
            }
        }
